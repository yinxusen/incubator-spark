 ...
 */
class GradientDescentAlt(var gradient: Gradient, var updater: Updater)
  extends Optimizer with Logging
{
  ...
  def optimize(data: RDD[(Double, Array[Double])], initialWeights: Array[Double])
    ...
    val (weights, stochasticLossHistory) = GradientDescentAlt.runMiniBatchSGD(
        data,
        gradient,
        updater,
        stepSize,
...
object GradientDescentAlt extends Logging {
  /**
  ...
  def runMiniBatchSGD(
    ...
    for (i <- 1 to numIterations) {
       ...
       */
      assert(lossSum / miniBatchFraction + regVal > 0.0)
