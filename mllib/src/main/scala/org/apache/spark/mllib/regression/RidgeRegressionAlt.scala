 ...
 */
class RidgeRegressionWithSGDAlt private (
    var stepSize: Double,
    var numIterations: Int,
    var regParam: Double,
    var miniBatchFraction: Double)
    extends GeneralizedLinearAlgorithm[RidgeRegressionModelAlt]
  with Serializable {

  val gradient = new SquaredGradient()
  val updater = new SquaredL2Updater()

  @transient val optimizer = new GradientDescentAlt(gradient, updater).setStepSize(stepSize)
  ...
  def createModel(weights: Array[Double], intercept: Double) = {
    ...
    new RidgeRegressionModelAlt(weightsScaled.data, interceptScaled)
  }

  override def run(
      input: RDD[LabeledPoint],
      initialWeights: Array[Double])
    : RidgeRegressionModelAlt =
  {
 ...
 */
object RidgeRegressionWithSGDAlt {
  ...
  def train(
    ...
    : RidgeRegressionModelAlt =
  {
    new RidgeRegressionWithSGDAlt(stepSize, numIterations, regParam, miniBatchFraction).run(
  ...
  def train(
    ...
    : RidgeRegressionModelAlt =
  {
    new RidgeRegressionWithSGDAlt(stepSize, numIterations, regParam, miniBatchFraction).run(input)
  }
   ...
   * @param regParam Regularization parameter.
   * @param numIterations Number of iterations of gradient descent to run.
   * @return a RidgeRegressionModelAlt which has the weights and offset from training.
   */
  def train(
      input: RDD[LabeledPoint],
      numIterations: Int,
  ...
  def main(args: Array[String]) {
    ...
    val model = RidgeRegressionWithSGDAlt.train(data, args(4).toInt, args(2).toDouble,
        args(3).toDouble)
